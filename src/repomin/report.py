...
def _provenance_markdown(result: ReductionResult) -> str:
    if result.stats.semantic_reducer == "http":
        semantic_boundary = (
            "An external HTTP semantic reducer proposed edits; every accepted "
            "edit was still checked by the configured failure oracle."
        )
    else:
        semantic_boundary = (
            "The external semantic reducer was disabled; ReproMin used its local "
            "reducers and checked every accepted edit with the configured failure "
            "oracle."
        )
    return (
        "## Provenance\n\n"
        "This metadata file was generated by ReproMin. It does not identify who "
        "selected the task, prepared the input, configured the failure oracle, "
        "invoked the tool, reviewed the result, or wrote surrounding analysis. "
        + semantic_boundary
        + "\n\nBefore sharing this artifact with another project, separately disclose "
        "any material LLM or agent involvement and follow that project's "
        "contribution policy.\n\n"
    )
