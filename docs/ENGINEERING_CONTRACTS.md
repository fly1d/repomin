...
This is the deep companion to the newcomer-focused
[contributor guide](../CONTRIBUTING.md). Read the sections for every subsystem a
change touches. Statements using **must**, **never**, or **require** are review
and compatibility requirements; they are not implementation suggestions.

Use the section headings to find contracts for report comparison, benchmarks,
cross-project outreach, reduction, sampling, holdout certification, reporting,
source mutation, manifest adapters, Java analysis, and verification. The
[architecture reference](ARCHITECTURE.md) explains the underlying design.
...
Use `REPORT_SCHEMA.md` as the contract source and update the README,
...
### Cross-project outreach

A public fixture or case study does not by itself authorize contacting another
project. Do not use automated cold outreach. Contact another project only after
a human has checked its contribution and AI-assistance policies and the
recipient has opted into the pilot. Disclose material agent involvement in the
first message. If the recipient accepts only human-created work or asks not to
...
[pydoctor outreach postmortem](CASE_STUDY_PYDOCTOR_728.md) records why this
rule is mandatory.

## Reduction, oracle, and execution contracts
...
## Repeated sampling and family control
...
## Holdout certification and recovery
...
## Reporting and phase accounting
...
## Coordinated Java changes and classpaths
